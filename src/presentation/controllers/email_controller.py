...
@email_controller.get("/send_email/{email}")
async def send_email(email_dto: str, email_service: IEmailServiceAbstract = Depends(build_email_service)):
    """
    Endpoint para enviar un correo electrónico.
    """
    try:
        await email_service.sendEmail_to_verify_email(EmailDTO(email=email_dto))
